...
logger.remove()
logger.add(lambda m: None, level=0)
...
def initialize_global_model():
    ...
    with _model_lock:
        ...
        try:
            import sys as _sys
            _sys.modules['__main__'].Mini = Mini
        ...
        try:
            ...
            if resp_json.get('result') == True:
                ...
                if validate_decoded and len(validate_decoded.strip()) > 10:
                    ...
                    if server_success:
                        logger.success(validate_decoded.strip())
                    else:
...
_token_queue = pyqueue.Queue(maxsize=200)
_solver_running = False
_solver_threads = []
_solver_init_lock = threading.Lock()

def _solver_worker(thread_id, config):
    d = None
    cycle = 0
...
def start_solver(num_threads=10):
    ...
    with _solver_init_lock:
        ...
        for i in range(num_threads):
            ...
            t = threading.Thread(target=_solver_worker, args=(i + 1, config), daemon=False)
            t.start()
...
if __name__ == '__main__':
    start_solver(int(input("Number of threads: ").strip()))
    logger.success(f"Solver started with _token_queue")
    while True:
        try:
            time.sleep(60)
